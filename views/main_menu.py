...
class MainMenu(tk.Frame):
    ...
    def __init__(self, master, device_manager, user_manager):
        ...
        self.tabs = ["Dispositivos", "Historial", "Agregar", "Telegram", "Configuración", "Cerrar Sesión"]
        self.tab_buttons = {}
        for t in self.tabs:
            b = tk.Button(
                self.tab_frame, 
        ...
        if TELEGRAM_AVAILABLE:
            ...
            # Frame placeholder si Telegram no disponible
            no_telegram_frame = tk.Frame(self.content, bg=COLORS["background"])
            tk.Label(
                no_telegram_frame,
                text="⚠️ Telegram no está disponible",
            ...
            tk.Label(
                no_telegram_frame,
                text="Para habilitar Telegram, instala:\npip install python-telegram-bot",
                bg=COLORS["background"],
                font=("Arial", 11),
                fg=COLORS["text_dark"]
            ).pack()
            self.frames["Telegram"] = no_telegram_frame
    ...
    def _show_frame(self, frame_name):
        ...
        # Mostrar frame solicitado
        if frame_name in self.frames:
            self.frames[frame_name].pack(fill="both", expand=True)
            
            # Si es Telegram, actualizar estado
            if frame_name == "Telegram" and hasattr(self.frames[frame_name], '_update_status'):
                self.frames[frame_name]._update_status()
